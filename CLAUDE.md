# Claude Context for Mouser MCP Server
...
**Example workflow**: When researching APIs or documentation:
...
This is a Mouser Electronics MCP (Model Context Protocol) server built with Python and FastMCP. It provides programmatic access to Mouser's electronic component search, cart management, and order tracking through the Model Context Protocol.
...
├── client.py     # Mouser API client (dual key authentication)
├── types.py      # TypedDict definitions for Mouser API responses
├── api/          # Mouser API modules
│   ├── search.py        # Component search operations
...
Each API module in `src/mouser_mcp/api/` should:
...
When adding new Mouser API functionality:

1. Create a new module in `src/mouser_mcp/api/` or extend existing ones
2. Define async functions with proper docstrings
3. Add TypedDict definitions in `types.py` matching Mouser's API response format
4. Register tools in `server.py`
5. Write tests in `tests/`
6. Ensure proper error handling for API failures
7. Update this documentation if patterns change

## Mouser API Specifics

### Dual API Keys

Mouser uses two separate API keys:
- **MOUSER_PART_API_KEY**: For search operations (`search.py`)
- **MOUSER_ORDER_API_KEY**: For cart, order, and history operations

The client automatically selects the correct key based on the endpoint.
...
Handle rate limit errors gracefully and inform users when limits are exceeded.
...
- `MOUSER_PART_API_KEY`: Part search API key from Mouser
- `MOUSER_ORDER_API_KEY`: Order/cart API key from Mouser
- `MOUSER_API_BASE_URL`: Base URL for Mouser API (default: https://api.mouser.com/api/v1)
- `MOUSER_API_TIMEOUT`: Request timeout in seconds
- `MOUSER_DEBUG`: Enable debug logging
...
3. Check for syntax errors: `uv run python -m py_compile src/mouser_mcp/server.py`
4. Verify both Mouser API keys are configured in `.env`
